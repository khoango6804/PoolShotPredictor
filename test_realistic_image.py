#!/usr/bin/env python3
"""
Test YOLOv11 model on realistic billiards image
"""
...
def test_realistic_image(model_path, image_path, conf=0.01):
    """Test YOLOv11 on realistic billiards image"""
    
    print(f"🎯 Testing YOLOv11 on realistic billiards image...")
    ...
    if results and len(results) > 0:
        ...
        if boxes is not None and len(boxes) > 0:
            ...
            expected_balls = 14  # 1 cue + 13 numbered balls
    ...
    # Save result image
    if results and len(results) > 0:
        output_path = f"realistic_billiards_result.jpg"
...
def test_multiple_confidences_realistic(model_path, image_path):
    """Test with multiple confidence levels"""
    
    print(f"🔍 Testing multiple confidence levels on realistic image...")
    print("=" * 60)
    ...
    for conf in confidence_levels:
        ...
        result = test_realistic_image(model_path, image_path, conf)
        results_summary[conf] = result
    
    # Summary
    print(f"\n📊 COMPREHENSIVE SUMMARY:")
    print(f"{'Confidence':<12} {'Detections':<12} {'Detection Rate':<15} {'Classes':<30}")
    print("-" * 75)
    for conf, result in results_summary.items():
        ...
        print(f"{conf:<12} {detections:<12} {detection_rate:<15.1f}% {class_str:<30}")

def main():
    print("🎯 YOLOv11 Realistic Billiards Testing")
    print("=" * 50)
    
    model_path = "runs/detect/yolo11_billiards_gpu/weights/best.pt"
    image_path = "realistic_billiards_test.jpg"
    ...
    if not Path(image_path).exists():
        ...
        print(f"Please run create_realistic_billiards_image.py first")
        return
    
    # Test with multiple confidence levels
    test_multiple_confidences_realistic(model_path, image_path)
    
    print(f"\n🎉 Realistic billiards testing completed!")
